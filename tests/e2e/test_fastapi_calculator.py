...
def test_homepage(base_url: str):
    url = f"{base_url}"
    response = requests.get(url)
...
def test_dashboard(base_url: str):
    ...
    # Parse the HTML content
    soup = BeautifulSoup(dash_response.text, "html.parser")

    # Extract data
    
    assert soup.title.string == "Dashboard"
